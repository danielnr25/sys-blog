...
class Libreria
{
    ...
    public function generarPaginacion($lista, $pagina, $filas, $entidad)
    {
        ...
        $puntosDelante = "";
        $puntosDetras  = "";
        $cadenaPagina .= "<div class='flex items-center w-full justify-between max-w-2xl mt-4'>";
        $cadenaPagina .= "<div class='active mr-4'><a href='#' id='divtotalregistros'>TOTAL DE REGISTROS: " . $cantidadTotal . "</a></div><div class='flex items-center space-x-2'>";

        for ($i = 1; $i <= $div; $i++) {
            if ($i == 1) {
                if ($i == $pagina) {
                    $cadenaPagina .= "<div class='bg-blue-corp w-8 h-8 flex items-center justify-center cursor-pointer rounded-lg text-white'><a class='w-full h-full flex items-center justify-center'>" . $i . "</a></div>";
                } else {
                    $cadenaPagina .= "<div class='w-8 h-8 flex items-center justify-center rounded-lg border border-gray-300 text-gray-500 cursor-pointer'><a class='w-full h-full flex items-center justify-center' onclick=\"buscarCompaginado(" . $i . ",'', '" . $entidad . "')\">" . $i . "</a></div>";
                }
            }
            if ($i == $div && $i != 1) {
                if ($i == $pagina) {
            ...
            if ($i > 1 && $i < ($pagina - 2)) {
                if ($puntosDelante == '') {
                    $puntosDelante =  "<div class='page-item disabled'><a class='w-full h-full flex items-center justify-center' href='#'>...</a></div>";
                    $cadenaPagina .= $puntosDelante;
                }
            }
            if ($i < $div && $i > ($pagina + 2)) {
                if ($puntosDetras == '') {
                    $puntosDetras = "<div class='page-item disabled'><a class='w-full h-full flex items-center justify-center' href='#'>...</a></div>";
                    $cadenaPagina .= $puntosDetras;
                }
            }
        }
        $cadenaPagina .= "</div></div>";
        $paginacion = array(
	...
	*/
    ...
    public static function formatDateFromExcel($date)
    {
